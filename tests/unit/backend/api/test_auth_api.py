...
class TestAuthAPI:
    ...
    def test_refresh_token(self, client, auth_headers):
        """Test token refresh - requires refresh token, not access token"""
        response = client.post('/api/v1/auth/refresh', headers=auth_headers)
        
        # This will fail with 422 because we're using access token instead of refresh token
        # In a real implementation, we'd need to get a refresh token from login
