...
class TestPokemonAPI:
    ...
    def test_get_pokemon_list_favorites_sorting_authenticated(self, client, auth_headers, test_user_id):
        ...
        assert data['pokemon'][1]['pokemon_id'] == 150  # Mewtwo
    
    def test_get_pokemon_list_invalid_sort(self, client):
        """Test Pokemon list with invalid sort parameter"""
        response = client.get('/api/v1/pokemon?sort=invalid')
        assert response.status_code == 200
    ...
    def test_get_pokemon_list_invalid_page(self, client):
        """Test Pokemon list with invalid page number"""
        response = client.get('/api/v1/pokemon?page=999')
        assert response.status_code == 200
    ...
    def test_get_pokemon_list_combinations(self, client, auth_headers, test_user_id):
        """Test Pokemon list with multiple filters combined"""
        # Add a favorite
        client.post(f'/api/v1/users/{test_user_id}/favorites', 
                   headers=auth_headers,
                   json={'pokemon_id': 25})  # Pikachu
        ...
        response = client.get('/api/v1/pokemon?sort=favorites&search=pika', headers=auth_headers)
        assert response.status_code == 200
        ...
        response = client.get('/api/v1/pokemon?sort=favorites&type=electric', headers=auth_headers)
        assert response.status_code == 200
